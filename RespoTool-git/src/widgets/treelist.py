...
class Treelist(ttk.Frame):
    ...
    def focus_index(self, index, visible=False):
        rows = self.tree.get_children()
        row_count = len(rows)
        if -row_count <= index < row_count:
            item = rows[index]
            self.focus_item(item, visible)

    def focus_item(self, item, visible=False):
        try:
            self.focus_force()
            ...
            logger.warning(f"Error in Treelist.focus_item(item={item}): {e}")
